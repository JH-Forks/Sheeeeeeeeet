...
open class ActionSheet: UIViewController {
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        self.headerConfiguration = .standard
        self.presenter = nil
        self.selectAction = { _, _ in print("itemSelectAction is not set") }
    ...
    public typealias SelectAction = (ActionSheet, MenuItem) -> Void
    ...
    open func present(in vc: UIViewController, from view: UIView?, completion: @escaping () -> Void = {}) {
        ...
        presenter?.present(sheet: self, in: vc.rootViewController, from: view) {
            completion()
            print(self.itemsTableView.contentSize)
        }
    }

    open func present(in vc: UIViewController, from item: UIBarButtonItem, completion: @escaping () -> Void = {}) {
        ActionSheet.applyAppearance(force: false)
        refresh()
        presenter = presenter ?? Self.defaultPresenter(for: vc)
    ...
    open func handleTap(on item: MenuItem) {
        ...
        dismiss { self.selectAction(self, item) }
    }
...
private extension ActionSheet {
    ...
    func totalHeight(for items: [MenuItem]) -> Double {
        items.reduce(0) {
            #if os(visionOS)
            $0 + $1.actionSheetCellHeight + 4 // visionOS UITableViewRowData._rowSpace == 4
            #else
            $0 + $1.actionSheetCellHeight
            #endif
        }
    }
}
